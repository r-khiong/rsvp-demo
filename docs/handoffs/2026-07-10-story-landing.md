...
2026 年 PM 招聘市場現實：AI 讓「能動的 demo」不再稀缺，招聘方篩選的是 outcomes、judgment、visible artifacts。RSVP-Demo 的核心價值（決策紀錄、scope 紀律、SDLC artifact chain）目前全部位於面試官不可見的位置——90% 功能在 admin login 之後、PM 文件不在 repo 裡。面試官點開履歷 URL 只看到報名表單，3 分鐘內無法辨識作者的 judgment。
...
> **文案唯一事實源。** 視覺依據：Figma file `RSVP-Demo` → frame `Story landing / v3`（含 Renata 編排後的分鏡素材）。
