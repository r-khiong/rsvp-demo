...
| Hosting | Vercel | - | Production deploy 唯一目標 |
...
| commit body 宣稱「跨 package 相容」但 tsc 未實際驗到 | 新裝跨層依賴（resolver / adapter / wrapper 類）後，在新 file 之外寫一個極簡引用點跑 tsc；確認跨 package 型別互通後再宣稱相容，不靠 single-file tsc pass 推論 |
...
Ship demo URL：4 個 user story (RSVP-3 ~ RSVP-6) E2E 跑通 + Vercel production deploy + README + Repo public。

### 7.2 Task Backlog（剩餘）

| Task | Scope | Status |
|---|---|---|
| RSVP-3 | shadcn install + Supabase 串接 + validation + redirect + error UI + E2E | In Progress |
| RSVP-4 | admin auth + login + list + filter + batch approve/reject | TODO |
| RSVP-5 | status page 三狀態 + QR generation + token 機制 | TODO |
| RSVP-6 simplified | manual check-in + search + status badge + check-in API | TODO |
| Deploy & Polish | E2E + responsive + UI polish + Vercel deploy + README + Repo public | TODO |
...
- Multi-language i18n
- Dark mode
- Real QR scanner（Sprint v2 用 manual check-in）
